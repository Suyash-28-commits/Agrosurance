...
const ContextProvider = (props) => {
  ...
  const newChat = () => {
    setLoading(false);
    setShowResult(false);
    setResultData(""); // Clear previous result on new chat
  };

  const onSent = async (prompt) => {
    setResultData(""); // Clear result data at the beginning of a new request
    setLoading(true);
    setShowResult(true);
    setRecentPrompt(prompt !== undefined ? prompt : input);

    try {
      const response = await run(prompt !== undefined ? prompt : input);
